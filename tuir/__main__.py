...
try:
    ...
    if sys.platform == "win32":
        sys.exit(
            "Fatal Error: This program is not compatible with Windows "
            "Operating Systems."
        )
    else:
        sys.exit(
            "Fatal Error: Your python distribution appears to be missing "
...
def main():
    ...
    try:
        ...
        _logger.warning(
            "Error parsing system locale: `%s`," " falling back to utf-8",
            default_locale,
        )
        encoding = "UTF-8"
    ...
    if not encoding or encoding.lower() != "utf-8":
        text = (
            "System encoding was detected as (%s) instead of UTF-8"
    ...
    if webbrowser_import_warning:
        _logger.warning(
            "webbrowser module was unexpectedly imported before"
    ...
    try:
        with curses_session() as stdscr:
            ...
            if config["link"]:
                ...
                url = requests.head(
                    config["link"], headers=reddit.http.headers, allow_redirects=True
                ).url
        ...
        exit_message = "\n".join(
            [
                ...
                "https://github.com/binodv/tuir/issues\n",
            ]
        )
